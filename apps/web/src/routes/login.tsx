import { createFileRoute, Link, useNavigate } from "@tanstack/react-router";
import { useState } from "react";
import { authClient } from "../lib/auth-client";
import { Button } from "#/components/ui/button";
import { Input } from "#/components/ui/input";
import { ServerSelector } from "#/components/server-selector";

export const Route = createFileRoute("/login")({
  component: LoginComponent,
});

function LoginComponent() {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError("");
    try {
      const res = await authClient.signIn.email({
        email,
        password,
      });
      if (res.error) {
        setError(res.error.message || "登录失败，请检查账号密码");
      } else {
        void navigate({ to: "/dashboard" });
      }
    } catch (err: any) {
      setError(err?.message || "登录时发生错误");
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="flex min-h-screen items-center justify-center p-4 md:p-8 bg-canvas font-sans relative">
      {/* 顶部或侧边的服务器选择控制区 */}
      <div className="absolute top-4 right-4 z-40">
        <ServerSelector />
      </div>

      <div className="w-full max-w-4xl flex flex-col gap-6">
        <div className="w-full bg-surface rounded-lg border border-hairline shadow-soft overflow-hidden animate-in fade-in duration-200">
          <div className="grid md:grid-cols-2">
            {/* 左侧逻辑登录表单 */}
            <form
              className="p-8 md:p-12 space-y-6 flex flex-col justify-center"
              onSubmit={handleLogin}
            >
              <div className="flex flex-col items-center gap-2 text-center">
                <h1 className="text-display text-ink font-semibold">欢迎回来</h1>
                <p className="text-meta text-muted text-balance">登录您的 OpenSlock 协作账号</p>
              </div>

              {error && (
                <div className="p-3 text-sm text-error bg-error/10 rounded border border-error/20">
                  {error}
                </div>
              )}

              {/* 输入区域 */}
              <div className="space-y-4">
                <div className="space-y-1">
                  <label className="block text-meta text-muted font-medium" htmlFor="email">
                    邮箱地址
                  </label>
                  <Input
                    id="email"
                    type="email"
                    required
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder="name@example.com"
                  />
                </div>

                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <label className="block text-meta text-muted font-medium" htmlFor="password">
                      安全密码
                    </label>
                    <a
                      href="#"
                      className="text-meta text-muted hover:text-ink hover:underline transition-colors"
                    >
                      忘记密码？
                    </a>
                  </div>
                  <Input
                    id="password"
                    type="password"
                    required
                    value={password}
                    onChange={(e) => setPassword(e.target.value)}
                    placeholder="请输入密码"
                  />
                </div>
              </div>

              <div className="space-y-4 pt-2">
                <Button type="submit" disabled={loading} className="w-full">
                  {loading ? "登录中..." : "安全登录"}
                </Button>

                {/* 分隔符 */}
                <div className="relative flex py-2 items-center">
                  <div className="flex-grow border-t border-hairline"></div>
                  <span className="flex-shrink mx-4 text-[11px] text-muted-soft text-caption-upper font-medium bg-surface px-2 select-none">
                    或使用第三方账号
                  </span>
                  <div className="flex-grow border-t border-hairline"></div>
                </div>

                {/* 社交账号多合一快捷登录 */}
                <div className="grid grid-cols-3 gap-3">
                  <Button variant="outline" type="button" className="h-10 rounded-md">
                    <svg className="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                      <path
                        d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.546 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09zM15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.78.896-1.454 2.338-1.273 3.714 1.338.104 2.715-.688 3.559-1.701"
                        fill="currentColor"
                      />
                    </svg>
                    <span className="sr-only">Apple 登录</span>
                  </Button>
                  <Button variant="outline" type="button" className="h-10 rounded-md">
                    <svg className="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                      <path
                        d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z"
                        fill="currentColor"
                      />
                    </svg>
                    <span className="sr-only">Google 登录</span>
                  </Button>
                  <Button variant="outline" type="button" className="h-10 rounded-md">
                    <svg className="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                      <path
                        d="M6.915 4.03c-1.968 0-3.683 1.28-4.871 3.113C.704 9.208 0 11.883 0 14.449c0 .706.07 1.369.21 1.973a6.624 6.624 0 0 0 .265.86 5.297 5.297 0 0 0 .371.761c.696 1.159 1.818 1.927 3.593 1.927 1.497 0 2.633-.671 3.965-2.444.76-1.012 1.144-1.626 2.663-4.32l.756-1.339.186-.325c.061.1.121.196.183.3l2.152 3.595c.724 1.21 1.665 2.556 2.47 3.314 1.046.987 1.992 1.22 3.06 1.22 1.075 0 1.876-.355 2.455-.843a3.743 3.743 0 0 0 .81-.973c.542-.939.861-2.127.861-3.745 0-2.72-.681-5.357-2.084-7.45-1.282-1.912-2.957-2.93-4.716-2.93-1.047 0-2.088.467-3.053 1.308-.652.57-1.257 1.29-1.82 2.05-.69-.875-1.335-1.547-1.958-2.056-1.182-.966-2.315-1.303-3.454-1.303zm10.16 2.053c1.147 0 2.188.758 2.992 1.999 1.132 1.748 1.647 4.195 1.647 6.4 0 1.548-.368 2.9-1.839 2.9-.58 0-1.027-.23-1.664-1.004-.496-.601-1.343-1.878-2.832-4.358l-.617-1.028a44.908 44.908 0 0 0-1.255-1.98c.07-.109.141-.224.211-.327 1.12-1.667 2.118-2.602 3.358-2.602zm-10.201.553c1.265 0 2.058.791 2.675 1.446.307.327.737.871 1.234 1.579l-1.02 1.566c-.757 1.163-1.882 3.017-2.837 4.338-1.191 1.649-1.81 1.817-2.486 1.817-.524 0-1.038-.237-1.383-.794-.263-.426-.464-1.13-.464-2.046 0-2.221.63-4.535 1.66-6.088.454-.687.964-1.226 1.533-1.533a2.264 2.264 0 0 1 1.088-.285z"
                        fill="currentColor"
                      />
                    </svg>
                    <span className="sr-only">Meta 登录</span>
                  </Button>
                </div>
              </div>

              <p className="text-meta text-center text-muted font-normal">
                还没有账号？{" "}
                <Link to="/register" className="font-semibold text-ink-soft hover:underline">
                  立即注册
                </Link>
              </p>
            </form>

            {/* 右侧极其精致的 ElevenLabs 编辑审美艺术壁纸 */}
            <div className="relative hidden bg-surface-strong md:block border-l border-hairline">
              <img
                src="https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=800&q=80"
                alt="ElevenLabs Aesthetic"
                className="absolute inset-0 h-full w-full object-cover grayscale opacity-90 contrast-[1.05]"
              />
              {/* 沉浸式壁纸名句或产品导言 */}
              <div className="absolute inset-0 bg-gradient-to-t from-ink/30 via-transparent to-transparent flex flex-col justify-end p-8 text-white select-none">
                <div className="space-y-1.5 drop-shadow-md">
                  <p className="text-display text-white font-semibold">OpenSlock</p>
                  <p className="text-meta text-white/80 leading-relaxed font-sans max-w-[280px]">
                    高密度与排版美学，打造自主托管的流式智能协作中心。
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* 页脚说明与免责声明 */}
        <p className="px-6 text-center text-meta text-muted-soft leading-normal font-sans">
          点击继续，即代表您同意我们的{" "}
          <a href="#" className="underline underline-offset-2 hover:text-muted transition-colors">
            服务条款
          </a>{" "}
          和{" "}
          <a href="#" className="underline underline-offset-2 hover:text-muted transition-colors">
            隐私政策
          </a>
          。
        </p>
      </div>
    </div>
  );
}
