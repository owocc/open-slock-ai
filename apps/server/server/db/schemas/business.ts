...
export const channelGroups = pgTable(
  "channelGroups",
  {
    ...
    serverId: text("serverId")
      ...
      .references(() => servers.id, { onDelete: "cascade" }),
    name: text("name").notNull(),
    sortIndex: integer("sortIndex").default(0).notNull(),
    createdAt: timestamp("createdAt", { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [
    index("idx_channelGroups_serverId").on(table.serverId),
    unique("channelGroups_serverId_name_unique").on(table.serverId, table.name),
  ],
);
...
export const channels = pgTable(
  "channels",
  {
    ...
    serverId: text("serverId")
      .notNull()
      .references(() => servers.id, { onDelete: "cascade" }),
    groupId: text("groupId").references(() => channelGroups.id, { onDelete: "set null" }),
    name: text("name").notNull(),
  ...
  (table) => [
    ...
    index("idx_channels_groupId").on(table.groupId),
...
export const channelGroupsRelations = relations(channelGroups, ({ one, many }) => ({
  server: one(servers, {
    fields: [channelGroups.serverId],
    references: [servers.id],
  }),
  channels: many(channels),
}));
